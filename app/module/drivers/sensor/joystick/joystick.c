...
int joy_trigger_set(const struct device *dev, const struct sensor_trigger *trig,
    ...
    k_timer_start(&drv_data->timer, K_MSEC(1000 / drv_cfg->frequency),
                  K_MSEC(1000 / drv_cfg->frequency));
...
int joy_init(const struct device *dev) {
    ...
    drv_data->as = (struct adc_sequence){
        .channels = BIT(drv_cfg->io_channel +
                        1), /* Has to be channel +1 because channel 0 is used for the battery */
    ...
    drv_data->acc = (struct adc_channel_cfg){.gain = ADC_GAIN_1_4,
